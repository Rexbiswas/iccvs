...
const Navbar = () => {
    ...
    const mobileBorder = useTransform(scrollProgress, transitionRange, ["rgba(241, 245, 249, 1)", "rgba(241, 245, 249, 1)"]);
    ...
    return (
        <>
            ...
            <motion.nav
                ...
                ref={navRef}
                className="hidden lg:flex fixed top-0 left-0 right-0 z-1000 px-6 lg:px-8 xl:px-10 py-1.5 items-center justify-between pointer-events-auto w-full"
            >
                {/* Left: Logo */}
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-[400px] h-16 z-[1005] lg:hidden">
                <div className={`w-full h-full flex items-center justify-around px-6 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.15)] transition-all duration-500 border ${isOpen ? 'bg-white border-slate-200' : 'bg-white border-white'}`}>
                    ...
                    <button
                        ...
                        className={`relative flex flex-col items-center justify-center w-16 h-16 transition-all duration-500 scale-90 active:scale-75 ${isSocialMenuOpen ? 'text-primary' : 'text-slate-900/40'}`}
                    >
                        {isSocialMenuOpen && (
                            <motion.div
                                layoutId="activeBubble"
